...
import static com.sakurafuld.kataklysm.Deets.KATAKLYSM;
...
public class AnchorScreen extends AbstractContainerScreen<AnchorMenu> {
    private static final ResourceLocation ANCHOR_LOCATION =
            new ResourceLocation(KATAKLYSM, "textures/gui/container/anchor.png");
    private int frame;
    private String name = "";
    ...
    public void removed() {
        ...
        this.menu.level.getBlockEntity(this.menu.pos, ModBlockEntities.ANCHOR.get()).ifPresent(anchorBlockEntity -> {
            ...
            PacketHandler.INSTANCE.send(PacketDistributor.SERVER.noArg(), new ServerboundAnchorUpdate(this.menu.pos, this.name));
        });
        super.removed();
    }
    ...
    public boolean keyPressed(int key, int scanCode, int modifier) {
        if((key == GLFW.GLFW_KEY_ESCAPE || key == GLFW.GLFW_KEY_ENTER) && this.shouldCloseOnEsc()){
            this.onClose();
            return true;
        }

        if (this.field.keyPressed(key))
            return true;
    ...
    public void renderField(PoseStack poseStack){
        ...
        int alpha = (int) (this.minecraft.options.getBackgroundOpacity(0.25f) * 255) << 24;
...
//            this.minecraft.font.drawInBatch(this.name, halfWidth, 0, nameColor, false, matrix, bufferSource, false, 0, 15728880, false);
            ...
            this.minecraft.font.drawInBatch(name, halfWidth, 0, nameColor, false, poseStack.last().pose(), this.minecraft.renderBuffers().bufferSource(), true, alpha, LightTexture.pack(15, 15));
            this.minecraft.font.drawInBatch(name, halfWidth, 0, nameColor, false, poseStack.last().pose(), this.minecraft.renderBuffers().bufferSource(), false, 0, LightTexture.pack(15, 15));

            if(cursor >= 0 && blink){
                int tail = this.minecraft.font.width(this.name.substring(0, Math.min(cursor, this.name.length())))
                           - this.minecraft.font.width(this.name) / 2;
                if(this.name.isEmpty()){ tail -= 2; poseStack.translate(-0.6d, 0, 0);}
                if(cursor >= this.name.length()) this.minecraft.font.drawInBatch("_", tail, 0, boardColor, false, matrix, bufferSource, false, 0, 15728880, false);
        ...
        if (this.name != null && cursor >= 0) {
            int j3 = this.minecraft.font.width(name.substring(0, Math.min(cursor, name.length())));
